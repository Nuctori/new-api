...
export const channelFormSchema = z
  .object({
    ...
    responses_compat_mode: z.boolean().optional(),
...
export function transformChannelToFormDefaults(
  ...
  if (channel.settings) {
    try {
      ...
      responsesCompatMode = parsed.responses_compat_mode === true
  ...
  return {
    ...
    responses_compat_mode: responsesCompatMode,
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  if (formData.type === 1 || formData.type === 43) {
    settingsObj.responses_compat_mode = formData.responses_compat_mode === true
  } else if ('responses_compat_mode' in settingsObj) {
    delete settingsObj.responses_compat_mode
  }
